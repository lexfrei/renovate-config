...
- Automerge (squash) for **every** update type, majors included, performed by Renovate itself rather than by GitHub auto-merge (`platformAutomerge: false`). CI is the gate: Renovate merges a branch only when it is up to date with the base branch and passing all tests, so a major that breaks the build stays open. A repository that wants a human on majors overrides `automerge` for `matchUpdateTypes: ["major"]` locally, and a repository with no CI at all needs `ignoreTests: true` or nothing will ever merge. GitHub auto-merge only exists for a PR that is not yet mergeable, so on a branch with no required status check the API rejects enabling it with `Pull request is in clean status`, and in a private repository below GitHub Pro the setting cannot be turned on at all — `PATCH /repos/{owner}/{repo}` accepts `allow_auto_merge: true` and leaves it false. Renovate does not fall back from either case, so those PRs sat open and green indefinitely. Doing the merge itself works the same way with or without branch protection. A repository that wants the platform behaviour can still set `platformAutomerge: true`, and needs a required check for it to mean anything.
...
- `pip_requirements` widened to the `.in` sources that `pip-compile` locks are generated from. The stock patterns match the compiled `.txt` only, so a bump rewrote the lock and left the source pinned to the old version — harmless where nothing reads the source, CI-red in a repo whose tests assert the two agree, and in both cases the file that documents the intended version stops being true.
